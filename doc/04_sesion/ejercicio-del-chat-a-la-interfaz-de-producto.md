...
1. Acepte un body de tipo `EstimationRequest`.

2. Llame a `render_estimation_prompt(request)` para obtener `(system, user)`.

3. Llame al modelo con `role: "system"` y `role: "user"` separados (no concatenados en un único mensaje).

4. Devuelva un `EstimationResponse` con el texto y `prompt_version="v1"`.
...
1. Que el render incluye literalmente el contenido de `description` dentro del bloque `<project_description>` (o `## Project description` si optas por Markdown).

2. Que cuando `output_format=phases_table` el system contiene la palabra clave del formato (por ejemplo, `"phases_table"` o `"confidence_pct"` si lo mencionas en las instrucciones), y que cuando es `narrative` no la contiene.

3. Que cuando `detail_level=detailed` el system incluye la instrucción extra de listar asunciones por fase, y que cuando es `summary` esa instrucción no aparece.
...
**Plazo de entrega.** Envía el enlace a tu rama a George a través de: [george@lidr.co](mailto:george@lidr.co), con al menos dos días de antelación a la sesión en vivo. Las entregas posteriores no se podrán incluir en la revisión grupal del inicio del directo.
